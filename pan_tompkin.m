...
x1 = load('e1.dat'); % load the ECG signal from the file/ result of CRLS
fs = 250;              % Sampling rate 200Hz
...
%%There is some filtering and normalization for visualizaiton.
%%if you want to comment out line 34-68,please add x6=x1 before line 69.
%LOW PASS FILTERING
% LPF (1-z^-6)^2/(1-z^-1)^2
...
y=[0:length(x6)-1]/fs;
[pks,locs]=findpeaks(x6,'MinPeakDistance',90);
[pmks,ilocs]=findpeaks(-x6,'MinPeakDistance',90);
hold on
figure(2);
subplot(2,1,1);
plot([0:length(x6)-1]/fs,x6,y(locs),pks,'o','MarkerFaceColor','r','MarkerSize',5);
...
subplot(2,1,2);
plot(d);
title('Plotting The AECG for comparsion');
%Heart Rate= (fs*60)/y;
% nobh=length(pks);
% tl=length(x6)/fs;
% hr=(nobh*60)/tl;
